...
import type { IAgentRuntime, Memory, EmbeddingModelSettings, ModelProviderName, IMemoryManager, UUID } from './types.js';

const defaultMatchThreshold = 0.1;
const defaultMatchCount = 10;
const DEFAULT_MAX_CONVERSATION_LENGTH = 20;
...
export class MemoryManager implements IMemoryManager {
    ...
    async createMemory(memory: Memory, unique = false): Promise<void> {
        ...
        if (!memory.id) {
            logger.warn("Attempted to check existence of memory without an ID.");
    ...
    async getMemoriesByRoomIds(params: { roomIds: UUID[], limit?: number; }): Promise<Memory[]> {
        if (!this.runtime.databaseAdapter) {
            logger.warn("Database adapter is not available in getMemoriesByRoomIds");
            return [];
        }
    ...
    async getMemoryById(id: UUID): Promise<Memory | null> {
        if (!this.runtime.databaseAdapter) {
            logger.warn("Database adapter is not available in getMemoryById");
            return null;
        }
    ...
    async removeMemory(memoryId: UUID): Promise<void> {
        if (!this.runtime.databaseAdapter) {
            logger.warn("Database adapter is not available in removeMemory");
            return;
        }
    ...
    async removeAllMemories(roomId: UUID): Promise<void> {
        if (!this.runtime.databaseAdapter) {
            logger.warn("Database adapter is not available in removeAllMemories");
            return;
        }
    ...
    async countMemories(roomId: UUID, unique = true): Promise<number> {
        if (!this.runtime.databaseAdapter) {
            logger.warn("Database adapter is not available in countMemories");
            return 0;
        }
